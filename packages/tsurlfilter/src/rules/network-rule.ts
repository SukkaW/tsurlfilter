...
export class NetworkRule implements rule.IRule {
    ...
    constructor(ruleText: string, filterListId: number) {
        const node = NetworkRuleParser.parse(ruleText, { isLocIncluded: false });
    ...
    private loadOption(optionName: string, optionValue: string, exception = false): void {
        ...
        // FIXME: keep this approach for now, or it would be better to create a map from names -> bit mask positions?
        // Once we have a bitmask position, we can use it instead of string names, because working with numbers is
        // faster
