import { CssTree, CssTreeParserContext } from '@adguard/agtree';
import { type SelectorListPlain } from '@adguard/ecss-tree';
import { hasValueWithBackslash, validateSelectorList } from '../../../src/rules/css/selector-tools';

describe('CSS selector tools', () => {
    describe('validateSelectorList - valid cases', () => {
        ...
        ])('should work for %s', (selectorList, isExtendedCss) => {
            const selectorListNode = CssTree.parsePlain(
                selectorList,
                CssTreeParserContext.selectorList,
            ) as SelectorListPlain;

            expect(validateSelectorList(selectorListNode)).toEqual({
                isValid: true,
                isExtendedCss,
            });
        });
    ...
    describe('validateSelectorList - invalid cases', () => {
        ...
        ])('should work for %s', (selectorList, isExtendedCss, errorMessage) => {
            const selectorListNode = CssTree.parsePlain(
                selectorList,
                CssTreeParserContext.selectorList,
            ) as SelectorListPlain;
    ...
    describe('hasValueWithBackslash', () => {
        test.each([
            // Doesn't contain backslash
            {
                actual: '{ background: url(http://example.org/image.png) }',
                expected: false,
            },
            {
                actual: '{ padding-left: 10px !important; }',
                expected: false,
            },
            // Contains backslash
            {
                actual: '{ background: \\75rl("http://example.org/empty.gif") }',
                expected: true,
            },
            {
                actual: '{ background:\\x75rl("http://example.org/image.png"); }',
                expected: true,
            },
        ])('should return $expected for $actual', ({ actual, expected }) => {
            const blockNode = CssTree.parse(actual, CssTreeParserContext.block);
            expect(hasValueWithBackslash(blockNode)).toBe(expected);
        });
    });
});
